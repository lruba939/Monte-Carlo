...
int main(){
    ...
    da_pos.open("omega0_Ra_1_pos.dat", std::fstream::out);
    da_stat.open("omega0_Ra_1_stat.dat", std::fstream::out);
        dyf_and_abs(Nmax, tmax, dt, xs, ys, D, omega[0], xr, yr, Rr, xa, ya, Ra[1], &da_pos, &da_stat);
    da_pos.close();
    da_stat.close();
